...
class Route {
	...
	public function new(f, consumes, produces) {
		...
		switch [hasCall(f), hasSub(f)] {
			...
			case [true, false]:
				kind = KCall({
					statusCode: switch field.meta.extract(':statusCode') {
						case []:
							macro 200;
						case [{params: [v]}]:
							v;
					...
					html: switch field.meta.extract(':html') {
						case []:
							None;
						...
						for (field in fields)
							switch field.target {
								case ATParam(kind):
									arr.push({
										id: id++,
										access: Drill(arg.name, field.name),
										type: field.type,
										kind: kind,
										meta: field.meta
									});
	...
	public static function getRestricts(meta:Array<MetaAccess>):Array<Expr> {
		return [
			for (meta in meta)
				for (m in meta.extract(':restrict'))
					switch m.params {
						case [v]:
							v;
						case _:
...
abstract Payload(Pair < Position, Array < {
	id:Int,
	access:ArgAccess,
	type:Type,
	kind:ParamKind,
	? meta : haxe.macro.Type.MetaAccess
}
	>>) {
	public inline function new(pos, arr)
		this = new Pair(pos, arr);
		...
		for (item in arr) {
			function add(to:Array<Field>, name:String) {
				...
				for (xMeta in xmlMeta) {
					if (item.meta != null && item.meta.has(xMeta)) {
			...
			switch item.kind {
				case PKBody(None):
					if (body.length > 0)
						pos.error('Body appeared more than once');
	...
	public function toObjectDecls() {
		var body = [];
		var bodyObj = EObjectDecl(body);
		var query = [];
		var queryObj = EObjectDecl(query);
		var header = [];
